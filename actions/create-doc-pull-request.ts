...
export default defineAction({
  description:
    "Open a draft pull request on BuilderIO/agent-native applying a tracked doc suggestion's before/after edits to the real .mdx source file.",
  schema: z.object({
  ...
  run: async ({ suggestionId }, ctx) => {
    ...
    try {
      file = await githubRuntime.readGitHubRepositoryFile({
        ...
        path: filePath,
        // Explicit: always start from main's current content, never whatever
        // state an earlier attempt's branch happens to be sitting at.
    ...
    // Verify every change before mutating anything — never open a partial or
    // garbage PR against the real repo. Check all of them (not just the
    // first failure) so a mismatch reports the full picture.
    const matchResults = changes.map((change, i) => ({
      index: i,
      change,
      range: findNormalizedRange(file.content as string, change.before),
    }));
    ...
    // Apply back-to-front so earlier ranges stay valid as later ones splice
    // in replacement text of a different length.
    ...
    const baseSha = (refResult.response?.json as { object?: { sha?: unknown } } | undefined)?.object
      ?.sha;
    if (!refResult.response?.ok || typeof baseSha !== "string") {
      fail(`Failed to read the ${GITHUB_DEFAULT_BRANCH} branch from GitHub: ${githubErrorMessage(refResult, "unknown error")}`, {
        statusCode: 502,
      });
    }
    ...
    if (!createRefResult.response?.ok && !branchAlreadyExists) {
      fail(`Failed to create a branch on GitHub: ${githubErrorMessage(createRefResult, "unknown error")}`, {
        statusCode: 502,
      });
    }
    if (branchAlreadyExists) {
      // A branch from an earlier, now-abandoned attempt (e.g. a closed,
      // unmerged PR) can still be sitting at a stale commit. Reset it to the
      // current main HEAD so the file sha we just read from main actually
      // matches what's on the branch — otherwise the write below 409s.
    ...
    const prJson = prResult.response?.json as { number?: unknown; html_url?: unknown } | undefined;
